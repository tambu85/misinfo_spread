""" Fitting script """
####come fitting.py, ma plotta la funzione ob in funzione di Nmax, il parametro che fa sballare tutto
####lo abbiamo fatto per trovare i minimi locali in cui finisce
...
def fitone(data, modelfunc, n_vars, bounds, nrep=25, fity0=False,
    ...
    return best_res.x, _stderr(best_res), _resid(data)
...
if __name__ == '__main__':
    ...
    tauinv = 0.1
    alpha = 0.5
    t = numpy.linspace(0, 200, 201)
    sigma = 0
    y0 = (0, 1000, 0, 0, 9000)
    syntdata = gendata(models.hoaxmodel, y0, t, sigma, pv, tauinv, alpha)
    syntdata = syntdata[:, :3]
    syntdf = pandas.DataFrame.from_records(syntdata)
    syntdf.columns = ["time","fact","fake"]

    Nmaxrange = range(10000,11000,500)
    fobT=[]
    fobF=[]

    for N_max in Nmaxrange :
        ...
        xopt1, err1, fob1 = fitone(syntdata, models.hoaxmodel, 5, bounds, fity0=True, nrep=1)
        numpy.set_printoptions(precision=2, suppress=True)
        print("{} +/- {}".format(numpy.round(xopt1, 2), numpy.round(err1, 2)))

        fobT.append(fob1)

        # [fa_opt, ba_opt, bi_opt, fi_opt, s_opt, pv_opt, tauinv_opt, alpha_opt] = numpy.round(xopt1, 2)
        # y0 = (fa_opt, ba_opt, bi_opt, fi_opt, s_opt)
        # sigma = 0
        ...
        # title = "FITTING synthetic data "
        # fig.suptitle(title)
        #
        # ax1 = plt.subplot(1, 2, 1)
        # syntdf['fake'].plot(legend=True, color='b', marker='.', ls='None', ax=ax1, title="Fake")
        # fit_df['ba_fit'].plot(legend=True, color='lightgrey', ls='-', ax=ax1)
        ...
        # syntdf['fact'].plot(legend=True, color='r', marker='.', ls='None', ax=ax2, title="Factcheck")
        # fit_df['fa_fit'].plot(legend=True, color='lightgrey', ls='-', ax=ax2)
        ...
        xopt2, err2, fob2 = fitone(syntdata, models.hoaxmodel, 5, bounds, nrep=1)
        numpy.set_printoptions(precision=2, suppress=True)
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2)
    title = "Finding bad N_max "
    fig.suptitle(title)

    ax1 = plt.subplot(1, 2, 1)
    plt.plot(Nmaxrange,fobT, ax=ax1)
    ax1.set_xlabel('N_max')
    ax1.set_ylabel('residfunc')


    ax2 = plt.subplot(1, 2, 2)
    plt.plot(Nmaxrange,fobF,ax=ax2)
    ax2.set_xlabel('N_max')
    ax2.set_ylabel('residfunc')
