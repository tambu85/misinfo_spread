...
# TODO change path of other filename
    ...
    if url_dict[keyurl] > 1000:
        ...
        for i in range(len(hoaxy_data)):
            if(snopes_url[i] == keyurl):  # for every single selected topic...
                ...
                # create two dictionaries, FOR and AGAINST with list of users
        ...
        min_date = min(min(for_time_dict.keys()),
                       min(against_time_dict.keys()))
        max_date = max(max(for_time_dict.keys()),
                       max(against_time_dict.keys()))
        print(min_date)
        print(max_date)
        ...
        for keytime in for_time_dict:
            df.loc[i] = [keytime, float(len(set(for_time_dict[keytime]))),
                         float(len(set(against_time_dict[keytime])))]
            i += 1
